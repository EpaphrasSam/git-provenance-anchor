...
1. Confirm baseline: `gpa reverify` reports `ok` for `v0.4.0-m4` on Arbitrum
   Sepolia and zkSync Sepolia (anchored tree `bfc4700b…`).
...
3. Run `gpa reverify --network arbitrumSepolia --network zkSyncSepolia`.
4. Restore the tag from origin: `git fetch origin tag v0.4.0-m4 --force`.
5. Re-run reverify and confirm `v0.4.0-m4` is `ok` again.

OP Sepolia was omitted from the attack arm because event listing timed out on
the public RPC during the baseline run; the property under test is independent
of that network once any live anchor exists.
...
Unaffected tags (`v0.1.0-m1` … `v0.3.0-m3`) stayed `ok`.
...
Re-verification detects the tj-actions-style tag retarget. The on-chain record
did not change; the local tag did. That mismatch is exactly the detective
control Chapter 3 describes: prevention of a force-push is a hosting-platform
concern, while detection is what this system contributes.

Reproduce:

```bash
gpa reverify --network arbitrumSepolia --network zkSyncSepolia
