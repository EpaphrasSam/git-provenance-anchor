...
OVERRIDES = {
    ...
    "AV-301": {
        "tag+snapshots": (
            "out-of-scope-with-record",
            "The injection is still faithfully anchored rather than caught: a "
            "snapshot of a poisoned branch records the poison. What changes is that "
            "branch-tip consumers gain something to verify against, and a commit "
            "later reverted or force-pushed away leaves an immutable trace. Useful "
            "for forensics, not detection.",
        )
    },
    "AV-302": {
        "tag+snapshots": (
            "out-of-scope-with-record",
            "As AV-301. Maintainer-level injection into sources is recorded, not "
            "detected.",
        )
    },
}
...
def main():
    ...
    # --- cadence: how long a branch-tip consumer goes unanchored -------------
    ...
    for repo in sample["repositories"]:
        ...
        cadence.append({
            ...
            "blindWindowTagOnlyDays": gap,
            "blindWindowWeeklySnapshotsDays": min(gap, 7),
            "windowReductionDays": max(0, gap - 7),
        })
    ...
    per_anchor = {n: fees["networks"][n]["anchorCostEth"]["p50"] for n in
                  ("arbitrumOne", "opMainnet", "zkSyncEra")}
    weekly = 52
    cost = {
    ...
    print("\nBlind window, tag-only versus weekly snapshots (days)")
    for c in cadence:
        print(f"  {c['repo']:34s} {c['medianTagGapDays']:6.0f} -> {c['blindWindowWeeklySnapshotsDays']:3.0f}"
              f"   saves {c['windowReductionDays']:5.0f}")
